...
def get_items(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    items = db.query(TodoItemModel).offset(skip).limit(limit).all()
    return items
...
@app.patch("/items/{item_id}", response_model=TodoItem)
def partial_update_item(item_id: int, item: TodoCreate, db: Session = Depends(get_db)):
    db_item = db.query(TodoItemModel).filter(TodoItemModel.id == item_id).first()
    if not db_item:
        raise HTTPException(status_code=404, detail="Item not found")
